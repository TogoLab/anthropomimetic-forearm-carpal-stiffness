...
stl/        3Dプリント用STLデータ（手根骨・靭帯治具・指先モールド）
media/      補足動画
```
...
本リポジトリの内容（CADデータ、STLデータ、ファームウェア、解析コード、補足動画）はすべて著者らによる自作データであり、**クリエイティブ・コモンズ 表示 4.0 国際ライセンス（CC BY 4.0）**の下で提供されます。  
使用する際は、関連論文の引用または本リポジトリURLの記載など、適切なクレジット表示をお願いします。  
ライセンス全文は `LICENSE.txt` を参照してください。
